...
- When applying Quantitative Input Influence (QII) in layer-by-layer fashion, a pattern of value would show up across levels of layers as well as neurons units within same layer. The context of experiment is any sophisticated model defined as inheritance of neural network and its variants.
...
In practice, I made an observation that if the loss of model is not low enough, it seems to be the case that value in both Shapley and Banzhaf for each feature is zero. This could be explained that an immature model works no better than a random outcome of a coin flip, or it's just simply that the size of dataset too small that could not make the model fit well. To push this hypothesis further, we first ensure the ``loss <= 0.5``, still, it's possible to obtain a sequence of zero value here as well. However, in order to minimize the probability this event happens, we ensure that the ``loss <= 0.5`` with ``epoch = 1000``, trained with ``140`` instances and test with ``10`` instances. We see no need to use 10-folds to test the model performance, for the primary object here is training dataset.
...
Even though aforementioned hypothesis is false, I believe this direction is worth being explored deeper. What if to achieve an acceptable accuracy that could work well not only on training but also test set, one must reach a safe range of accuracy/loss which good enough to be underfitting and not too good to be overfitting. However, with obtained model, it could not produce any non-zero Shapley/Banzhaf value if it could not reach a certain accuracy level, which is higher than predefined safe zone. Here we can see a trade-off between overfitting and obtaining meaningful result of influence scores. Apparently, this hypothesis does not apply on current model, but we can try on different data set or architecture.
...
As mentioned, QII could analyze not meaningful feature as units in input, but also any layer in Neural Network class. The question is, while they are eligible to calculate, those latent (unexplainable) units are not self-explanatory and it's difficult to extract observation from. A natural extension to this is mapping those values back to original input via bridges made of weights. As point out, most of the weights share similar pattern, hence we expect whatever the value is, it would keep extending in that direction. Furthermore, biases play an important role in classification, still, they are dead-end cause 1) there is no inverse-map for non-linear function relu and softmax and 2) there is no connection from previous layer to current bias unit. However, it's worth noting to test this on fully linear neural network in the future. 

A successful mapback requires 3 parameters:
- Quantity of Interest: no change ratio, ``no``, w.r.t. observed data point, or difference ratio, ``diff``.
...
- Map back technique: current layer matrix multiplicates to transpose (``trans``) or inverse (``inv``) weights that connects to previous layer. One can interprete the operation with transpose matrix as distributing the score proportionally to previous layer. Besides, we can preprocess the weight before doing that with ``softmax`` (normalize weight to probabilty distribution form), ``shift`` (shift all value to positive side such that lowest value is 0) or both - ``shift_softmax``. The intuition behind shift is that even though if obtained Shapley/Banzhaf value is negative, one can simply shift them to positive side, to show that any agent in coalition always contribute something. 
...
For a same number of n elements, apparently number of permutations is bigger than number of subsets could be constructed from. Therefore, if one calculates Banzhaf and Shapley exhaustively, Banzhaf is definitely a winner, in terms of time performance. On the other hand, if one do the approximation, I will fix number of sampled permutations or subsets and number of drawn samples both as ``600``, hence no significant gap between two methods.

I first compare the time performance with a random data point in table below, for each layer in the model with exhaustive computation using ``no_keep_trans`` mapback. Hidden layer 1 takes most of the time is reasonable, because it has largest number of units in one layer. As expected, Banzhaf always needs lower time to execute, even though this is 1-time run only.
...
<p align="center">
  ...
  <br><br>
  <b>Figure 8: Shapley value through different layers.</b><br>
</p>

<p align="center">
  <img src="images/exp01/banzhaf.png">
  <br><br>
  <b>Figure 9: Banzhaf value through different layers.</b><br>
</p>

Figure 8, 9 shows consistency in both methods. Despite slight difference in value, the tendency is crystal clear. A drastic change in hidden layer 3 is notable, hence further investigation required. We would be back to this later.

### How good is sampling-based method

To evaluate actual Shapley value. in case of numerous features, one cannot simply wait until it finished. In order to address this issue, sampling-based method are proposed when drawn permutations/subsets and number of samples for each time evaluate quantity of interests from training dataset. We set ``n_perms`` \ ``n_subsets`` = 600 and ``n_pool_samples`` = 600 as well, to achieve confident level and accurary theoretically. Table below shows disrepency between 2 methods: exhaustive and approximation on hidden layer 1, the layer has most number of units.
...
This does not imply approximation can work well on big data set for we have ``n_pool_samples`` = ``size_train_dataset``. Fortunately, we are dealing with a small dataset, hence following experiments would be evaluated with exhaustive method.
...
<p align="center">
  ...
  <br><br>
  <b>Figure 10: Observed feature pair.</b><br>
</p>

<p align="center">
  <img src="images/exp02/set 2/shapley.png">
  <br><br>
  <b>Figure 11: Mapback Shapley value of 7 typical points for 3 flower types, feature 1 and 4. Order: setosa-centroid, setosa-bound, versicolor-bound, versicolor-centroid, versicolor-bound, virginica-bound, virginica-centroid</b><br>
</p>

We define the set of important features includes ``feature 1`` and ``feature 4``. As shown in  Figure 11, a pattern of correlation between 2 features giving us a hint to determine the predicted class of data points. We also test with all features in each layers, but no clear trend can be identified (Figure 12). This is in our expectation, because set all features equally important is no other than calculate Global Shapley value of the model. However, by making further observations in all-feature set, we see clearer pattern in higher levels, especially hidden layer 3 (Figure 13). In hidden layer 2 and 3, less features active, hence we can select them as important features and see is there any difference would be made. Therefore, we compare selected features with all features in hidden layers. Unfortunately, Figure 14 shows no significant change when you emphasize the features on hidden layer 2 showing activeness in global evaluation. We achieve similar result with hidden layer 3, which does not show here.


<p align="center">
  <img src="images/exp02/set 4/shapley_mapback.png">
  <br><br>
  <b>Figure 12: Mapback Shapley value of 7 typical points for 3 flower types, all 4 features. Row order: setosa-centroid, setosa-bound, versicolor-bound, versicolor-centroid, versicolor-bound, virginica-bound, virginica-centroid. Column order: input, hidden layer 1, hidden layer 2, hidden layer 3</b><br>
</p>

<p align="center">
  <img src="images/exp02/set 4/shapley_original.png">
  <br><br>
  <b>Figure 13: Original value of 7 typical points for 3 flower types, all 4 features. Row order: setosa-centroid, setosa-bound, versicolor-bound, versicolor-centroid, versicolor-bound, virginica-bound, virginica-centroid. Column order: input, hidden layer 1, hidden layer 2, hidden layer 3</b><br>
</p>

<p align="center">
  <img src="images/exp03/shapley_manipulate_hidden2.png">
  <br><br>
  <b>Figure 14: Comparison of Shapley value between choose all features and feature 1 + 3 only in hidden layer 2. Row order: setosa-centroid, setosa-bound, versicolor-bound, versicolor-centroid, versicolor-bound, virginica-bound, virginica-centroid. Column order: 6 features-hidden layer 2, mapback to input layer, 2 features-hidden layer 2, mapback to input layer</b><br>
</p>
...
<p align="center">
  ...
  <br><br>
  <b>Figure 15: Mapback comparison with first row is original Shapley value in hidden layer 3 and other rows are corresponding combinations to table explained previously</b><br>
</p>
